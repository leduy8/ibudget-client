...
const TransactionDetails = (props) => {
  ...
  const onUpdateAfterDeletion = async () => {
    const temp: any = await transactInWallet(
      token,
      { price: focusTransaction.price * -1 },
      focusTransaction.wallet.id
    );
    ...
    <SafeAreaView style={styles.container}>
      <View style={{ backgroundColor: "#fff" }}>
        ...
        <View style={{ backgroundColor: "#fff" }}>
          <PlainItemBar
            iconName={
              categoryIconsMapper[`${focusTransaction.category.icon_name}`]
            }
            name={focusTransaction.category.name}
          />
          <PlainItemBar
            name={`${focusTransaction.category.type === "Income" ? "+" : " "}${
            ...
            customStylesText={{
              ...
              marginLeft: 10,
              color: `${
                focusTransaction.category.type === "Income" ? happy : frown
              }`,
            }}
          />
          ...
          />
          <PlainItemBar
            name={`Is this a positive transaction: ${
              focusTransaction.is_positive ? "Yes!" : "No."
            }`}
            children={
              <Icon
