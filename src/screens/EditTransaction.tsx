...
const EditTransaction = (props) => {
  ...
  const { focusTransaction } = useSelector(
    (state: any) => state.focusTransactionState
  );
  const [money, setMoney] = useState<any>(
    `${
      focusTransaction.price > 0
        ? focusTransaction.price
        : focusTransaction.price * -1
    }`
  );
  const [categories, setCategories] = useState<any>();
  const [focusCategory, setFocusCategory] = useState<any>(
    focusTransaction.category
  );
  const [focusCategoryIcon, setFocusCategoryIcon] = useState<any>(
    focusTransaction.category.icon_name
  );
  const [note, setNote] = useState(focusTransaction.note);
  const [datePicked, setDatePicked] = useState(
    getDateJsonFormat(getDateJsonFormat(focusTransaction.created_date))
  );
  const [walletPicked, setWalletPicked] = useState<any>(
    focusTransaction.wallet
  );
  ...
  const [transactionStatus, setTransactionStatus] = useState(
    Number(`${focusCategory.is_positive ? 1 : 2}`)
  );
  ...
  const onUpdateTransaction = async (transaction) => {
    ...
    const returnedWalletAfterTransaction: any = await transactInWallet(
      token,
      { price: updatedMoney - focusTransaction.price },
      walletPicked.id
    );
    ...
    const returnedTransaction: any = await updateTransaction(
      transaction,
      token,
      focusTransaction.id
    );
  ...
  const onDeleteOldAndCreateNewTransaction = async (transaction, oldId) => {
    const transactMoney =
      focusTransaction.category.type === "Expense" ? money * -1 : money;
    const returnedOldWalletAfterTransaction: any = await transactInWallet(
      token,
      { price: transactMoney * -1 },
      oldWallet.id
    );
    ...
    const returnedAfterDeleteTransaction: any = await deleteTransaction(
      token,
      oldId
    );
    ...
    const returnedNewWalletAfterTransaction: any = await transactInWallet(
      token,
      { price: transactMoney },
      walletPicked.id
    );
    ...
    const returnedCreateTransaction: any = await createTransaction(
      transaction,
      token
    );
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              ...
              if (focusTransaction.wallet.id === walletPicked.id)
                ...
                await onDeleteOldAndCreateNewTransaction(
                  transaction,
                  focusTransaction.id
                );
